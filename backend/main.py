...
# --- USER MANAGEMENT (FIXED: ADDED MISSING ENDPOINTS) ---
...
async def create_batch(data: BatchSchema, authorization: str = Header(None)):
    ...
    try:
        ...
        ref_batch = db.reference(f'users/{uid}/batches')
...
async def get_batches(authorization: str = Header(None)):
    ...
    try:
        decoded_token = auth.verify_id_token(token)
        uid = decoded_token['uid']
        snapshot = db.reference(f'users/{uid}/batches').get()
        batches_list = []
        if snapshot:
...
async def update_batch(batch_id: str, data: BatchUpdateSchema, authorization: str = Header(None)):
    ...
    try:
        decoded_token = auth.verify_id_token(token)
        uid = decoded_token['uid']
        ref_batch = db.reference(f'users/{uid}/batches/{batch_id}')
...
async def delete_batch(batch_id: str, authorization: str = Header(None)):
    ...
    try:
        decoded_token = auth.verify_id_token(token)
        uid = decoded_token['uid']
        db.reference(f'users/{uid}/batches/{batch_id}').delete()
...
async def add_expense(data: ExpenseSchema, authorization: str = Header(None)):
    try:
        ...
        db.reference(f'users/{uid}/batches/{data.batchId}/expenses').push({
...
async def get_expenses(batch_id: str, authorization: str = Header(None)):
    try:
        token = authorization.split("Bearer ")[1]
        uid = auth.verify_id_token(token)['uid']
        snapshot = db.reference(f'users/{uid}/batches/{batch_id}/expenses').get()
...
async def update_expense_category(data: UpdateFeedCategorySchema, authorization: str = Header(None)):
    try:
        token = authorization.split("Bearer ")[1]
        uid = auth.verify_id_token(token)['uid']
        ref_exp = db.reference(f'users/{uid}/batches/{data.batchId}/expenses/{data.expenseId}')
...
async def add_sale(data: SalesRecordSchema, authorization: str = Header(None)):
    try:
        token = authorization.split("Bearer ")[1]
        uid = auth.verify_id_token(token)['uid']
        db.reference(f'users/{uid}/batches/{data.batchId}/sales').push({
            **data.dict(exclude={"batchId"}),
...
async def get_sales(batch_id: str, authorization: str = Header(None)):
    try:
        token = authorization.split("Bearer ")[1]
        uid = auth.verify_id_token(token)['uid']
        snapshot = db.reference(f'users/{uid}/batches/{batch_id}/sales').get()
        return [{"id": k, **v} for k, v in snapshot.items()] if snapshot else []
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
VITAMIN_LOGIC = [
    (range(1, 8), 35.0), (range(8, 22), 25.0), (range(22, 31), 15.0),
]
...
async def get_feed_forecast(batch_id: str, authorization: str = Header(None)):
    try:
        token = authorization.split("Bearer ")[1]
        uid = auth.verify_id_token(token)['uid']
        batch_data = db.reference(f'users/{uid}/batches/{batch_id}').get()
        ...
        for day in range(1, 31):
            ...
            v_match = next((item for item in VITAMIN_LOGIC if day in item[0]), None)
            if f_match:
                forecast_list.append({
                    "day": day, "feedType": f_match[2],
                    "targetKilos": round(f_match[1] * multiplier, 2),
                    "targetGrams": round(v_match[1] * multiplier, 2) if v_match else 0
                })
